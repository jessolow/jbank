...
CREATE POLICY "Users can view own account balances" ON account_balances
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE id = account_balances.account_id AND (
                (owner_type = 'CUSTOMER' AND owner_id LIKE auth.uid()::text || ':%') OR
                (owner_type = 'LOAN' AND EXISTS (
                    SELECT 1 FROM lending_core.term_loans 
                    WHERE loan_id::text = owner_id AND user_id = auth.uid()
...
-- Users can view their own LoC exposure
CREATE POLICY "Users can view own LoC exposure" ON lending_core.loc_exposure
    FOR SELECT USING (auth.uid() = user_id);

-- Users can view their own timeline
CREATE POLICY "Users can view own timeline" ON history.timeline_by_customer
...
CREATE OR REPLACE FUNCTION auth.has_service_role()
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION auth.has_service_role() TO authenticated;
